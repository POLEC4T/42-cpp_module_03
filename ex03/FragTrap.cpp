/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   FragTrap.cpp                                       :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: miloniemaz <mniemaz@student.42lyon.fr>     +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/12 01:19:06 by miloniemaz        #+#    #+#             */
/*   Updated: 2025/08/13 01:23:32 by miloniemaz       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "./FragTrap.hpp"

using namespace std;

FragTrap::FragTrap() {
    _hitPoints = _initHitPoints;
    _energyPoints = _initEnergyPoints;
    _attackDamage = _initAttackDamage;
    cout << "FragTrap " << _name << " created" << endl;
}

FragTrap::FragTrap(std::string name) : ClapTrap(name) {
    _hitPoints = _initHitPoints;
    _energyPoints = _initEnergyPoints;
    _attackDamage = _initAttackDamage;
    cout << "FragTrap " << _name << " created" << endl;
}

FragTrap::~FragTrap() {
    cout << "FragTrap " << _name << " destroyed" << endl;
}

FragTrap::FragTrap(const FragTrap &other) : ClapTrap(other) {
    cout << "FragTrap " << _name << " created (copy)" << endl;
}

FragTrap& FragTrap::operator=(const FragTrap &other) {
    if (this == &other)
        return (*this);
    ClapTrap::operator=(other);
    cout << "FragTrap " << _name << " created (assignment)" << endl;
    return (*this);
}

void FragTrap::attack(const std::string& target) {

    if (_energyPoints <= 0) {
        cout << "FragTrap " << _name 
             << " has no energy left to attack!" << endl;
        return;
    }
    if (_hitPoints <= 0) {
        cout << "FragTrap " << _name 
             << " cannot attack because it has no hit points left!" << endl;
        return;
    }
    _energyPoints--;
    cout << "FragTrap " << _name 
        << " attacks " << target 
        << ", causing " << _attackDamage 
        << " points of damage!" << endl;
}

void FragTrap::highFivesGuys() {
    cout << "FragTrap " << _name <<
        " is now in high fives mode!" << endl; 
}